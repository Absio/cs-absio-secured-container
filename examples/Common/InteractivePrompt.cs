...
namespace Common
{
    public static class InteractivePrompt
    {
        ...
        /// </summary>
        /// <param name="lambda">
        ///     This func is provided for the user to handle the input.  Input is provided in both string and List
        ///     &lt;char&gt;. A return response is provided as a string.
        /// </param>
        /// <param name="prompt">The prompt for the interactive shell</param>
        /// <param name="startupMsg">Startup msg to display to user</param>
        public static void Run(Func<string, List<char>, List<string>, string> lambda, string prompt, string startupMsg,
            List<string> completionList = null)
        {
            ...
            while (true)
            {
                ...
                do
                {
                    ...
                    else if (key.Key == ConsoleKey.Home || key.Key == ConsoleKey.H &&
                    ...
                    else if (key.Key == ConsoleKey.End || key.Key == ConsoleKey.E &&
                             key.Modifiers == ConsoleModifiers.Control)
                    {
                        inputPosition = input.Count;
                        var cursorLeft = 0;
                        var cursorTop = startingCursorTop;
                    ...
                    else if (key.Key == ConsoleKey.Enter && Console.KeyAvailable)
                    {
                        input.Insert(inputPosition++, '\n');
                        RewriteLine(input, inputPosition);
                    }
        ...
        {
            var cursorLeft = InputIsOnNewLine(input, inputPosition) ? 0 : _prompt.Length;
        ...
        {
            s.Add(input);
            var direction = key.Modifiers == ConsoleModifiers.Shift ? -1 : 1;
